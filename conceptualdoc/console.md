...
Using the .NET Execution environment (DNX), it's very easy to run a
...
source](https://github.com/aspnet/Docs/tree/master/samples/CreatingConsoleAppWithDNX)
from the project created in this article.
...
Creating a console application is extremely straightforward. For this
article, we're going to use the following C\# class, which has just one
...
> **note**
>
> `dnx` references several [environment
> variables](https://github.com/aspnet/Home/wiki/Environment-Variables),
> such as `DNX_TRACE`, that affect its behavior.
...
Creating and running your first console application on DNX is very
simple, and only requires two files.
